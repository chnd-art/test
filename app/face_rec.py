...
VONAGE_API_KEY = 'ac1c5569'
VONAGE_API_SECRET = 'Cbt8A5JBaJQIjUAG'
VONAGE_NUMBER = 'Vonage APIs'
RECIPIENT_NUMBER = '212628438353'
...
def send_sms(timestamp):
    logging.debug('Starting to send SMS')
    client = vonage.Client(key=VONAGE_API_KEY, secret=VONAGE_API_SECRET)
    sms = vonage.Sms(client)
    responseData = sms.send_message({
        "from": VONAGE_NUMBER,
        "to": RECIPIENT_NUMBER,
        "text": f"Alert: An unrecognized person was detected at {timestamp}.",
    })
...
# Function to save unrecognized faces
def save_unrecognized_face(image):
    if not os.path.exists('unrecognized_faces'):
        os.makedirs('unrecognized_faces')
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"unrecognized_faces/unknown_{timestamp}.png"
    ...
    def face_prediction(self,test_image, dataframe,feature_column,
        ...
        for res in results:
            ...
            if person_name == 'Unknown':
                logging.debug('Unrecognized person detected')
                face_image = test_image[y1:y2, x1:x2]
                file_path, timestamp = save_unrecognized_face(face_image)
                with open('unrecognized_faces/report.txt', 'a') as f:
